...
言界 0.8.0 固定言台标签`v1.0.0`和言据 1.1.2 对应的 Git 提交
...
入口保留公开控件类和薄方法，数据视图以及滚动、列表语义、标签页和分割面板算法位于独立
模块；最低工具链的发布门禁会同时构建包入口和全部示例，并拒绝任一序列化字节码模块超过
32 MiB。
...
各层门禁、正式运行器和应用上线清单见[生产验收](PRODUCTION_VALIDATION.md)。
