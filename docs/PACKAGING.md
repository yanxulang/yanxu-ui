...
言界 0.9.0 固定言台标签`v1.0.0`和言据 1.1.2 对应的 Git 提交
...
汇总后另起独立任务重新下载四个待发布附件，核对归档安全路径、清单和 API 冻结、历史兼容
与供应链证据、六套目标锁、运行时元数据及每个自动关闭 YXB 的内部摘要。演练成功后保留
包含来源提交、全部证据与锁摘要和六目标 YXB 字节数的报告。

正式 Release 工作流不重新编译。它只接受与标签同一提交的成功 CI run，下载已经验收的
汇总候选，复核 SHA-256 后发布。CI run 只有在独立候选演练成功后才会标记成功，因此标签、
源码、API 快照、六目标锁与 Release 制品具有同一提交来源。
